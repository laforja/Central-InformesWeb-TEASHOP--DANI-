...
Public Class IndiceDeVentasResumido
    ...
    Private Function InicializarGrid(ByVal strWhere As String) As DataSet
        ...
        If cmbVendedor.Value IsNot Nothing And cmbVendedor.Value <> "0" Then
            strSQL = "SELECT DATEADD(DAY, 0, DATEDIFF(day, 0, Estadisticas.FechaFactura)) AS FechaFactura, Vendedores.Nombre, " & _
                        "SUM(Estadisticas.ImporteFactura + Estadisticas.ImporteIva) AS Facturacion, " & _
                        "COUNT(DISTINCT Estadisticas.NumeroDeFactura) AS NTickets, " & _
                        "COUNT(Estadisticas.NumeroDeFactura) AS NLineas, " & _
                        "SUM(Estadisticas.ImporteFactura + Estadisticas.ImporteIva) / COUNT(DISTINCT Estadisticas.NumeroDeFactura) AS TicketMedio, " & _
                        "CONVERT(DECIMAL(16,2),COUNT(NumeroDeFactura)) / CONVERT(DECIMAL(16, 2), COUNT(DISTINCT NumeroDeFactura))  AS Upt, " & _
                        "AVG(Estadisticas.ImporteFactura + Estadisticas.ImporteIva) AS Pma " & _
                     "FROM Estadisticas INNER JOIN Vendedores ON Estadisticas.Vendedor = Vendedores.Vendedor "

            strGroupOrder = " GROUP BY Vendedores.Nombre, DATEADD(DAY, 0, DATEDIFF(day, 0, FechaFactura)) ORDER BY FechaFactura"
        Else
            strSQL = "SELECT DATEADD(DAY, 0, DATEDIFF(day, 0, Estadisticas.FechaFactura)) AS FechaFactura, '' AS Nombre, " & _
                       "SUM(Estadisticas.ImporteFactura + Estadisticas.ImporteIva) AS Facturacion, " & _
                       "COUNT(DISTINCT Estadisticas.NumeroDeFactura) AS NTickets, " & _
                       "COUNT(Estadisticas.NumeroDeFactura) AS NLineas, " & _
                       "SUM(Estadisticas.ImporteFactura + Estadisticas.ImporteIva) / COUNT(DISTINCT Estadisticas.NumeroDeFactura) AS TicketMedio, " & _
            ...
            strGroupOrder = " GROUP BY DATEADD(DAY, 0, DATEDIFF(day, 0, FechaFactura)) ORDER BY FechaFactura"
        End If
